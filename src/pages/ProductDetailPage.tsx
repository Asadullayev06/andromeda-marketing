...
export default function ProductDetailPage() {
  ...
  return <>
    ...
    <div className="grid-2 dossier-grid">
      <section className="card"><div className="card-title"><Warehouse size={18} /> {t('nav.warehouses')}</div>{data.warehouses.length ? <table className="data compact"><tbody>{data.warehouses.map((row) => <tr key={`${row.name}-${row.code}`}><td>{row.name}<div className="cell-sub">{row.code || '—'}</div></td><td className="num qty-strong">{fmtNum(row.quantity)}</td></tr>)}</tbody></table> : <EmptyState title={t('common.none')} />}</section>
      <section className="card"><div className="card-title"><CalendarDays size={18} /> {t('company.expiry.title')}</div>{data.expiries.length ? <table className="data compact"><thead><tr><th>{t('customs.expiry')}</th><th>{t('customs.batches')}</th><th className="num">{t('common.qty')}</th></tr></thead><tbody>{data.expiries.map((row, index) => <tr key={`${row.batch_number}-${index}`}><td>{fmtDate(row.expiry_date)}</td><td>{row.batch_number || '—'}</td><td className="num">{fmtNum(row.quantity)}</td></tr>)}</tbody></table> : <EmptyState title={t('common.none')} />}</section>
      ...
      <section className="card dossier-wide"><div className="card-title"><ShieldCheck size={18} /> {t('customs.title')}</div>{data.customs.length ? <table className="data compact"><thead><tr><th>{t('customs.invoice')}</th><th>{t('customs.regime')}</th><th>{t('customs.expiry')}</th><th className="num">{t('common.qty')}</th></tr></thead><tbody>{data.customs.map((row) => <tr key={row.id}><td>{row.invoice}</td><td>{row.regime}</td><td>{fmtDate(row.expiry_date)}</td><td className="num">{fmtNum(row.quantity)}</td></tr>)}</tbody></table> : <EmptyState title={t('common.none')} />}</section>
    </div>
  </>;
}
